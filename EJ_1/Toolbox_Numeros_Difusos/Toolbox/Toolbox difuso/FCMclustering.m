...
if isfield(param,'m') == 1        % Índice de difusidad
    m = param.m;
else
    m = 2;
end
...
% end
% 
if isfield(param,'DistanceMetric') == 1    % Ponderador
    distance_metric = param.DistanceMetric;
else
...
%% Iteraciones
% ops=[2, 100, 1e-5,0];
options = fcmOptions(NumClusters=c, ...
...
data_clusters = cell(2,1);
for i=1:c
    data_clusters{i}=NaN(N,n);
end

for i=1:length(Z(:,1))
    [~,loc]=max(U(i,:));
    data_clusters{loc}(i,:)= data.X(i,:);
end

for i=1:c
    data_clusters{i} = data_clusters{i}(~isnan(data_clusters{i}(:,1)),:);
end


varianza = zeros(c,length(Z(1,:)));
for i=1:c
    for j=1:length(Z(1,:))
